...
sealed trait Opts[A] {
  ...
  def map[B](fn: A => B) = mapValidated(fn andThen Parse.success)
...
object Opts {
  ...
  case class Single[A](opt: Opt[A], help: String) extends Opts[A]
  case class Validate[A, B](a: Opts[A], validation: A => Parse.Result[B]) extends Opts[B]
  ...
  def required[A : Read](long: String, metavar: String, help: String): Opts[A] =
    Single(Opt.Regular(long, metavar), help).mapValidated {
  ...
  def remainingArgs[A : Read](metavar: String): Opts[List[A]] =
    Single(Opt.Arguments(metavar, Int.MaxValue), "Unused.").mapValidated { list =>
