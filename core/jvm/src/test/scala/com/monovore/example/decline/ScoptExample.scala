    ...
    extends CommandApp(
      ...
      main = {
        ...
        val libMax = {
          ...
          val max =
            Opts
              .option[Int]("max", help = "Limit for --libname option.")
        ...
        val jars = Opts
          .options[Path]("jar", short = "j", help = "Jar to include! More args, more jars.")
          .orEmpty
        ...
        val debug = Opts
          .flag(
            "debug",
            "Debug mode: shows in full list, not in usage.",
        ...
        val update = Opts.subcommand("update", help = "A command! This is the command help text.") {
          ...
          val keepalive =
            Opts
              .flag("not-keepalive", help = "Disable keepalive?")
              .map { _ => false }
              .withDefault(true)
          ...
          (keepalive, xyz).tupled
            .validate("Can't both keepalive and xyz!") { case (keepalive, xyz) =>
              !(keepalive && xyz)
            }
        }
